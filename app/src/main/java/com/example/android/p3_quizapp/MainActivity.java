package com.example.android.p3_quizapp;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //XML resource, but that would be too complicated for this stage.
    ...
    public void onClickSubmit(View v) {
        ...
        //I found that using StringBuilder it's easier to use, since it can append several types of variables.
        ...
        for (int i = 0; i < answerCheckers.length; ++i) {
            ...
            if (answerState == AnswerState.ANSWER_EMPTY) {
                ...
                if (questionList.length() != 0) {
                    //Do not add a comma before the first element in the list, or
                    //add a comma after only before the second element
                    questionList.append(",");
                }
                questionList.append(" ");
        ...
        if (emptyAnswerCount != 0) {
            ...
            if (wrongAnswerCount != 0) {
                ...
                final StringBuilder answerSummary = new StringBuilder(
                        getString(R.string.answer_summary, name, NUM_QUESTIONS - wrongAnswerCount, NUM_QUESTIONS));

                //There are 4 types of messages that appear when you finish the quiz (all questions answered).
                //The 4 score ranges are I: between 0 and 4, II: between 5 and 7, III: 8 and 9, IV: all correct answers (10);
                final int SCORE_LOW = 6;
                final int SCORE_MEDIUM = 3;

                final String scoreMessage, emailScoreMessage;
                if (wrongAnswerCount >= SCORE_LOW) {
                    scoreMessage = getString(R.string.message_score_low);
                    emailScoreMessage = getString(R.string.email_score_low);
                } else if (wrongAnswerCount >= SCORE_MEDIUM) {
                    scoreMessage = getString(R.string.message_score_medium);
                    emailScoreMessage = getString(R.string.email_score_medium);
                } else {
                    scoreMessage = getString(R.string.message_score_high);
                    emailScoreMessage = getString(R.string.email_score_high);
                }
                answerSummary.append(scoreMessage);

                Toast.makeText(getApplicationContext(), answerSummary.toString(), Toast.LENGTH_LONG)
                        .show();
                ad.setMessage(answerSummary.toString());

                //Dialog button for reviewing the score if the player wants to see what the correct answers
                ad.setButton(BUTTON_NEGATIVE,
                        getString(R.string.dialog_button_negative_text),
                ...
                //Dialog button for sharing the score by e-mail
                final int correctAnwersCount = NUM_QUESTIONS - wrongAnswerCount;
                ad.setButton(BUTTON_POSITIVE,getString(R.string.dialog_button_positive_text),new DialogInterface.OnClickListener(){
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        Intent intent = new Intent(Intent.ACTION_SENDTO);
                        intent.setData(Uri.parse(getString(R.string.mailto)));
                        intent.putExtra(Intent.EXTRA_SUBJECT,getString(R.string.intent_extra_subject) + name);
                        intent.putExtra(Intent.EXTRA_TEXT, name + " " +
                                getString(R.string.intent_extra_text_2) + " " + correctAnwersCount + " " +
                                getString(R.string.intent_extra_text_3) + " " + NUM_QUESTIONS +  " " + getString(R.string.intent_extra_text_4) +
                                " " + name + " " + emailScoreMessage);
                ...
                final int correctAnwersCount = NUM_QUESTIONS - wrongAnswerCount;
                final String emailScoreMessage = getString(R.string.email_all_answers_correct);
                ...
                ad.setButton(BUTTON_NEUTRAL,
                        getString(R.string.intent_email_button_neutral_text),
                        new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                dialog.dismiss();
